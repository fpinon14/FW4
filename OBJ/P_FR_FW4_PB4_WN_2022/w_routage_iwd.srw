...
type pb_1 from u_8_pbretour within w_routage_iwd
end type
type pb_router from picturebutton within w_routage_iwd
end type
type dw_routage from datawindow within w_routage_iwd
end type
type dw_motif from datawindow within w_routage_iwd
end type
type gb_1 from groupbox within w_routage_iwd
end type
end forward
...
global w_routage_iwd w_routage_iwd
...
dwCodOper.SetTransObject(SQLCA)
lIdSin=Long(stPass.stab[2])

dwCodOper.Retrieve(lIdSin,sIdOper, stPass.sTab[1])
dwCodOper.Sort ()
...
lRow=dw_motif.Find("categorie ='A' and cod_mot ='A5'",1,dw_motif.rowCount())
If lRow > 0 Then
	dw_motif.setItem(lRow, "ALT_ERR","O")
	dw_motif.setItem(lRow, "COEFF",0)
End if
end event
...
type cb_debug from w_ancetre`cb_debug within w_routage_iwd
end type
...
integer height = 672
integer taborder = 10
string dataobject = "d_routage_iwd"
