HA$PBExportHeader$n_cst_word2010.sru
$PBExportComments$------} UserObjet pour WORD 2010.
forward
global type n_cst_word2010 from n_cst_word
end type
end forward

global type n_cst_word2010 from n_cst_word
end type
global n_cst_word2010 n_cst_word2010
...
// [MIGPB11] [EMD] : Debut : 'remont$$HEX1$$e900$$ENDHEX$$' de ces variables sous forme de constantes, dans la classe ancetre
	...
	constant String K_WORD2010		= "Word.Application"
	constant String K_WORD2016		= "Word.Application.16"
// [MIGPB11] [EMD] : Fin
...
public function integer uf_agrandirword (boolean abagrandir);//*-----------------------------------------------------------------
//*
//* Fonction		: n_cst_word2010::uf_AgrandirWord			(PUBLIC)
...
public subroutine uf_commandeword (integer aitype, string asparam1, ref oleobject aoleobject);//*-----------------------------------------------------------------
//*
//* Fonction		: n_cst_word2010::uf_CommandeWord		(PUBLIC)
//* Auteur			: Franck Musso
//* Date				: 22/09/2010
//* Libell$$HEX4$$e900090009000900$$ENDHEX$$: 
//* Commentaires	: Cr$$HEX3$$e900e9002000$$ENDHEX$$en se basant sur le modele de l'objet n_cst_Word_2003
//*
...
//*-----------------------------------------------------------------
//* MAJ      PAR      Date	  Modification
//* 			FPI	08/01/2015	[Wyse.Word2010] Correction plantage commande 10 & 6
//* 			JFF   17/01/2020	J'ajoute $$HEX2$$e0002000$$ENDHEX$$la section 6 la m$$HEX1$$ea00$$ENDHEX$$me lecture que la section 4 pour le redimensionnement de Word
//*-----------------------------------------------------------------
...
// [Office2010] [FMU] : Fin
	...
	//	aOleObject.Activate ()
		...
		// [OFFICE2010] Debut AJOUT LBP le 08/10 : !?...
		// ??? Temporisation pour laisser le temps $$HEX2$$e0002000$$ENDHEX$$l'objet OLE de traiter les commandes. ???
		//  comprends pas le pourqoui du comment mais si pas de tempo echec du placement 
		// des fen$$HEX1$$ea00$$ENDHEX$$tres sous Office 2010
		nDelay.of_wait(1)
		//[OFFICE2010] Fin
		
		// [Wyse.Word2010]
		bContinue=TRUE // Si erreur de positionnement, on sort sans erreur
		lTotFenetre = 0
		If (not isnull(aOleObject.Windows)) and isValid(aOleObject.Windows) Then 
			lTotFenetre = aOleObject.Windows.Count 
		End if
		// :[Wyse.Word2010]
	...
	/*----------------------------------------------------------------------------*/
		aOleObject.ActiveDocument.SaveAs2 ( asParam1 )
	Case 9
	/*------------------------------------------------------------------*/	
	...
	/*------------------------------------------------------------------*/
		If aOleObject.Documents.Count > 0 Then // [Wyse.Word2010]
			aOleObject.Documents.Close ( 0, 0, TRUE )
		End if
...
// Trappage des erreurs OLE
	...
	if OleE_1.Source <> "" then szError = szError + " Source=" + OleE_1.Source
	if OleE_1.Text <> "" then 
		szError = szError + " Text=" + OleE_1.Text
	End if

	szError = szError + " Ligne=" + String(OleE_1.Line)
...
public function integer uf_fichierouvertdansword (ref oleobject aoleobject, boolean abfermer, boolean abfermer_sansverifier);//*-----------------------------------------------------------------
//*
//* Fonction		: n_cst_word2010::uf_FichierOuvertDansWord		(PUBLIC)
//* Auteur			: Erick John Stark
//* Date				: 23/05/2006 10:03:22
//* Libell$$HEX4$$e900090009000900$$ENDHEX$$: 
...
/*------------------------------------------------------------------*/
//lFenetre = aOleObject.Windows.Count //[I037] Tentative de correction du bug report$$HEX2$$e9002000$$ENDHEX$$sur SIndi
// Les test n'ont pas permis de demontrer que cela marchait bien ou pas sans cette correction.
// Je la laisse dans le doute, car ca marche comme ca, et d'apr$$HEX1$$e800$$ENDHEX$$s l'aide en ligne VBA,
// cela semble plus orthodoxe cod$$HEX2$$e9002000$$ENDHEX$$comme cela.
lFenetre = aOleObject.Application.Windows.Count
/*------------------------------------------------------------------*/
...
public function integer uf_creeroleobject_word (ref oleobject aoleobject);//*-----------------------------------------------------------------
//*
//* Fonction		: n_cst_word2010::uf_CreerOleObject_Word		(PUBLIC)
//* Auteur			: Erick John Stark
//* Date				: 23/05/2006 10:03:22
//* Libell$$HEX4$$e900090009000900$$ENDHEX$$: 
...
If	uf_WordOuvert () = 0	Then
	iRet = aOleObject.ConnectToNewObject ( K_WORD2010 )
	if iRet < 0 Then iRet = aOleObject.ConnectToNewObject ( K_WORD2016 ) // [OFFICE2016]
Else
	iRet = aOleObject.ConnectToObject ( "", K_WORD2010 )
	if iRet < 0 Then iRet = aOleObject.ConnectToObject ( "", K_WORD2016 ) // [OFFICE2016]
...
public function integer uf_fermerword ();//*-----------------------------------------------------------------
//*
//* Fonction		: n_cst_word2010::uf_FermerWord		(PUBLIC)
//* Auteur			: Erick John Stark
//* Date				: 23/05/2006 10:03:22
//* Libell$$HEX4$$e900090009000900$$ENDHEX$$: 
...
on n_cst_word2010.create
call super::create
end on

on n_cst_word2010.destroy
call super::destroy
end on

event constructor;call super::constructor;// [Office2010] [FMU] : Debut : 
is_SousRepModele = 'W2K10\'
// [Office2010] [FMU] : Fin
